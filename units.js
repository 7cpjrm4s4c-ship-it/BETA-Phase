...
/* Zahlen sauber formatieren (kein Exponential, max. 8 signifikante Stellen) */
function ufmt(v, decimals) {
  ...
  if (abs >= 1)   return v.toLocaleString('de-DE', { maximumFractionDigits: 6, maximumSignificantDigits: 8 });
  const sig = parseFloat(v.toPrecision(8));
  return sig.toLocaleString('de-DE', { maximumFractionDigits: 10 });
}

function _buildSelects(cat) {
  const d  = UNITS[cat];
...
function unitSwap() {
  ...
  if (resText && resText !== '\u2013') {
    ...
    if (!isNaN(n)) { const fv = $('unit-from-val'); if (fv) fv.value = n; }
  }
  unitCalc();
}
...
document.addEventListener('DOMContentLoaded', () => {
  $('unit-cat-sel')?.addEventListener('change', e => unitCat(e.target.value));
  $('unit-from-val')?.addEventListener('input', unitCalc);
  $('unit-from-sel')?.addEventListener('change', unitCalc);
  $('unit-to-sel')?.addEventListener('change', unitCalc);
  $('unit-swap-btn')?.addEventListener('click', unitSwap);
  unitCat('pressure');
});
