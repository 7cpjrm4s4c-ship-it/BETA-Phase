/* ═══════════════════════════════════════════════════════
   pipe.js  —  TechCalc Pro
   Rohrdimensionierung: Darcy-Weisbach · Colebrook-White
   Abhängigkeit: app.js muss zuerst geladen sein ($, loc)
═══════════════════════════════════════════════════════ */
'use strict';

/* ─── ROHRTABELLE ─── */
/* [DN, di_s, da_s, wall_s, norm_s, di_m, da_m, wall_m]
   Stahl : DIN EN 10255 Reihe M (≤ DN 50) / DIN EN 10220 (≥ DN 65)
...
/* Hydraulikparameter (Wasser 60 °C) */
const RHO          = 983.2;      // kg/m³  — Dichte
const NU           = 0.474e-6;   // m²/s   — kinematische Viskosität
const ES           = 0.046e-3;   // m      — Rauheit Stahl
const EM           = 0.015e-3;   // m      — Rauheit Mapress
const DP0          = 100;        // Pa/m   — Auslegungsgrenze (Standard)
const MAPRESS_MAX  = 100;        // DN     — Geberit Mapress max
...
function pCardMapress(p, vol, mx, isBest, bestCls) {
  bestCls = bestCls || 'best';
  if (p[0] > MAPRESS_MAX || p[5] === null) {
    return `<div class="pm na">
      <div class="pm-std mp">Mapress Edelstahl</div>
      <div class="pm-dn">DN\u2009${p[0]}</div>
      <div class="pm-na-txt">Nicht verf\u00fcgbar<br>(max. DN\u2009${MAPRESS_MAX})</div>
    </div>`;
  }
...
function renderPair(cid, vol, mx, bestCls) {
  ...
  const iM = bM < 0
    ? PIPES.findIndex(p => p[0] === MAPRESS_MAX)
    : bM;
...
function calcPipeTab() {
  ...
  const lo = Math.max(0, Math.min(
    bS < 0 ? PIPES.length - 1 : bS,
    bM < 0 ? PIPES.length - 1 : bM
  ) - 1);
  const hi = Math.min(PIPES.length - 1, Math.max(
    bS < 0 ? PIPES.length - 1 : bS,
    bM < 0 ? PIPES.length - 1 : bM
  ) + 1);
...
document.addEventListener('DOMContentLoaded', () => {
  ['p-vol', 'p-dp'].forEach(id =>
    $(id)?.addEventListener('input', calcPipeTab)
  );
});
