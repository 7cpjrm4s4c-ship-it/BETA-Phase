...
function twFixtureInput(id, value='0') {
  return `<div class="iwrap tw-iwrap">
    <input class="inp-sm tw-in" id="${id}" type="number" min="0" step="1" value="${value}" inputmode="numeric" class="tw-inp--compact"/>
...
function twFixtureRows(prefix, counts={}) {
  return TW_FIXTURES.map(f => `
    <div class="tw-row tw-fixture-row">
      <div>
        <div class="tw-key--lg">${f.label}</div>
        <div class="tw-val--sm">V<sub>R</sub> ${twFmt(f.vr,2)} l/s ${f.dauer?'· Dauerverbraucher':''}</div>
      </div>
...
function renderTwNes(preserveFocus=true) {
  ...
  wrap.innerHTML = TW_STATE.nes.length ? TW_STATE.nes.map((ne, idx) => {
    ...
    const rows = TW_FIXTURES.map(f => `
      <div class="tw-ne-fixture-row">
        <div>
          <div class="tw-key">${f.label}</div>
          <div class="tw-val">V<sub>R</sub> ${twFmt(f.vr,2)} l/s</div>
        </div>
        ${twFixtureInput(`tw-ne-${ne.id}-${f.id}`, ne.counts[f.id] || 0)}
      </div>`).join('');
    return `<div class="gc tw-ne-card" data-ne-id="${ne.id}">
      <button type="button" class="tw-ne-head tw-free-toggle" data-act="toggle" data-id="${ne.id}">
        <span class="tw-free-toggle-icon">${open?'▼':'▶'}</span>
        <span>
          <span class="tw-ne-title-row">NE ${idx+1} · <span id="tw-ne-title-${ne.id}">${twNeTitle(ne)}</span></span>
          <span id="tw-ne-summary-${ne.id}" class="tw-ne-sub-row">${twNeSummary(ne)}</span>
        </span>
        <span type="button" class="tw-ne-del" data-act="remove" data-id="${ne.id}" class="tw-err">Löschen</span>
      </button>
      <div class="tw-ne-body ${open?'open':''}">
        ...
        <div class="igrp">
          ...
          <div class="iwrap"><input class="inp tw-ne-name" id="tw-ne-name-${ne.id}" data-id="${ne.id}" type="text" value="${twEsc(ne.name)}" placeholder="z. B. WC Damen EG" class="tw-inp--mid"/></div>
        </div>
        <div class="igrp">
          <div class="ilbl">Gleichzeitigkeitsansatz</div>
          <div class="tw-ne-item">
            <div class="info-txt" id="tw-ne-hint-${ne.id}">${def.hint}</div>
            <div class="iwrap"><input class="inp-sm tw-ne-gl" id="tw-ne-gl-${ne.id}" data-id="${ne.id}" type="text" inputmode="decimal" lang="de-DE" pattern="[0-9]*[,.]?[0-9]*" autocomplete="off" value="${String(Number(ne.gl || 0).toFixed(2)).replace('.', ',')}" ${def.editable?'':'disabled'} class="tw-inp--compact"/><span class="iunit">GL</span></div>
          </div>
        </div>
