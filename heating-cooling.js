/* ═══════════════════════════════════════════════════════
   ...
   Heizung · Kälte
   Rohrdimensionierung → pipe.js
   Abhängigkeit: app.js, pipe.js müssen zuerst geladen sein
═══════════════════════════════════════════════════════ */
'use strict';
...
const DP0 = 100; // Pa/m Auslegungsgrenzwert für Rohrempfehlung
...
/* Ausgabeelement mit Text füllen, ohne den <span class="u"> zu verlieren */
...
function setOutFull(valId, unitId, on, numTxt, unitTxt) {
  ...
  const uel = $(unitId);
  if (uel) uel.textContent = unitTxt;
}
...
function calcAll() {
  ...
  if (rH.ok) {
    show($('pi-h'), true);
    const ph = $('pi-h-vol');
    if (ph) ph.textContent = '(' + loc(rH.m3h, 3) + '\u202fm\u00b3/h)';
    if (typeof renderPair === 'function') renderPair('pi-h-pair', rH.m3h, DP0, 'best-h');
  } else {
    show($('pi-h'), false);
  }
  ...
  if (rK.ok) {
    show($('pi-k'), true);
    const pk = $('pi-k-vol');
    if (pk) pk.textContent = '(' + loc(rK.m3h, 3) + '\u202fm\u00b3/h)';
    if (typeof renderPair === 'function') renderPair('pi-k-pair', rK.m3h, DP0, 'best-k');
  } else {
    show($('pi-k'), false);
  }
}

/* ─── HEIZUNG ↔ KÄLTE UMSCHALTEN ─── */
function flowSwitch(hk) {
  show($('flow-panel-h'), hk === 'h');
  show($('flow-panel-k'), hk === 'k');
  const btnH = $('flow-btn-h');
  const btnK = $('flow-btn-k');
  if (btnH) btnH.className = 'hk-btn' + (hk === 'h' ? ' on-h' : '');
  if (btnK) btnK.className = 'hk-btn' + (hk === 'k' ? ' on-k' : '');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.mbtn[data-p]').forEach(b => {
    b.addEventListener('click', () => {
      ...
      ST[p].mode = v;
      document.querySelectorAll(`.mbtn[data-p="${p}"]`).forEach(x => x.classList.remove('active'));
  ...
  // W/kW Umschalter
  $('h-wu') ?.addEventListener('click', () => setQUnit('h', 'W'));
  $('h-kwu')?.addEventListener('click', () => setQUnit('h', 'kW'));
  $('k-wu') ?.addEventListener('click', () => setQUnit('k', 'W'));
  $('k-kwu')?.addEventListener('click', () => setQUnit('k', 'kW'));

  // Heizung/Kälte Umschalter
  $('flow-btn-h')?.addEventListener('click', () => flowSwitch('h'));
  $('flow-btn-k')?.addEventListener('click', () => flowSwitch('k'));
